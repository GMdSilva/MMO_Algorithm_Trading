...
class Strategies(Price_analysis):
    ...
    class Arbitrage(Price_analysis):
        def __init__(self, pc, order_type, method, item='coin'):
            ...
            if self.trade_method == 'simulation':
                ...
                self.staleness_threshold = 30000000000000
            else:
        ...
        def monitor_orders(self):
            ...
            if not self.found:
                if self.trade_method is 'wild' or self.trade_method is 'simulation':
                    if self.index >= 2 or (self.index > 0 and self.staleness_counter > self.staleness_threshold)/7:
                        self.is_order_stale = True
...
def seek_opportunity(percent, previous_price, current_price, order_type):
    if order_type == 'bid' and percent <= cons.THRESH_DOWN:
        print(f'Bought incorret price at {current_price}, when {previous_price} was expected. Difference: {percent}%')
        #game.click_accept(order_type)
        game.bye_confirmation_box()
        playsound(cons.AUDIO)
        #time.sleep(10)
    if order_type == 'ask' and percent >= cons.THRESH_UP:
        print(f'Sold incorret price at {current_price}, when {previous_price} was expected. Difference: {percent}%')
        #game.click_accept(order_type)
        game.bye_confirmation_box()
        playsound(cons.AUDIO)
        #time.sleep(10)
